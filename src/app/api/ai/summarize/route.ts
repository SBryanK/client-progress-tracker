...
export async function POST(req: Request) {
  try {
    // Owner-only: the corpus contains every weekly update for every
    // client in the configured OWNER_EMAILS pool, so anonymous callers
    // must not be able to query it.
    await requireOwner();
